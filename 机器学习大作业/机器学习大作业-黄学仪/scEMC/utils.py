...
import seaborn as sns  # 导入 seaborn，用于绘图时的配色
...
def GetCluster(X, res, n):
    ...
    if np.unique(Y_pred_init).shape[0] <= 1:
        ...
        exit("Error: There is only a cluster detected. The resolution:" + str(
            res) + "is too small, please choose a larger resolution!!")
    else:
