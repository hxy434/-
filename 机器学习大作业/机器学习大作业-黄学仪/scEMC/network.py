...
def buildNetwork2(layers, type, activation="relu"):
    ...
    for i in range(1, len(layers)):
        net.append(nn.Linear(layers[i - 1], layers[i]))  # 添加线性层
        net.append(nn.BatchNorm1d(layers[i], affine=True))  # 添加批量归一化层
        if activation == "relu":
            net.append(nn.ReLU())  # 添加 ReLU 激活函数
        elif activation == "selu":
            net.append(nn.SELU())  # 添加 SELU 激活函数
        elif activation == "sigmoid":
            net.append(nn.Sigmoid())  # 添加 Sigmoid 激活函数
        elif activation == "elu":
            net.append(nn.ELU())  # 添加 ELU 激活函数
    return nn.Sequential(*net)  # 返回神经网络层序列


class scEMC(nn.Module):
    """
    scEMC 网络类，继承自 nn.Module。
    """

    def __init__(self, input_dim1, input_dim2,
                 encodeLayer1=[], encodeLayer2=[],
        ...
        self.tau = tau  # 模糊参数
        self.input_dim1 = input_dim1  # 输入维度1
        self.input_dim2 = input_dim2  # 输入维度2
        self.cutoff = cutoff  # 截止阈值
        self.activation = activation  # 激活函数类型
        self.sigma1 = sigma1  # 噪声标准差1
        self.sigma2 = sigma2  # 噪声标准差2
        self.alpha = alpha  # 参数 alpha
        self.gamma = gamma  # 参数 gamma
        self.phi1 = phi1  # 参数 phi1
        self.phi2 = phi2  # 参数 phi2
        self.t = t  # 迭代次数
        self.device = device  # 设备类型
        self.encoder1 = buildNetwork2([input_dim1] + encodeLayer1, type="encode", activation=activation)  # 构建编码器1
        self.encoder2 = buildNetwork2([input_dim2] + encodeLayer2, type="encode", activation=activation)  # 构建编码器2
        self.decoder1 = buildNetwork2(decodeLayer1, type="decode", activation=activation)  # 构建解码器1
        self.decoder2 = buildNetwork2(decodeLayer2, type="decode", activation=activation)  # 构建解码器2
        self.dec_mean1 = nn.Sequential(nn.Linear(decodeLayer1[-1], input_dim1), MeanAct())  # 解码均值层1
        self.dec_disp1 = nn.Sequential(nn.Linear(decodeLayer1[-1], input_dim1), DispAct())  # 解码离散度层1
        self.dec_mean2 = nn.Sequential(nn.Linear(decodeLayer2[-1], input_dim2), MeanAct())  # 解码均值层2
        self.dec_disp2 = nn.Sequential(nn.Linear(decodeLayer2[-1], input_dim2), DispAct())  # 解码离散度层2
        self.dec_pi1 = nn.Sequential(nn.Linear(decodeLayer1[-1], input_dim1), nn.Sigmoid())  # 解码 pi 层1
        self.dec_pi2 = nn.Sequential(nn.Linear(decodeLayer2[-1], input_dim2), nn.Sigmoid())  # 解码 pi 层2
        self.zinb_loss = ZINBLoss()  # 零膨胀负二项分布损失
        self.z_dim = decodeLayer1[0]  # 解码层维度
        self.trans_enc = nn.TransformerEncoderLayer(d_model=2 * encodeLayer1[-1], nhead=1,
                                                    dim_feedforward=256)  # Transformer 编码层
        self.extract_layers = nn.TransformerEncoder(self.trans_enc, num_layers=1)  # Transformer 编码器
    ...
    def kmeans_loss(self, z):
        ...
        返回：
        - dist1: 距离矩阵
        - torch.mean(torch.sum(dist2, dim=1)): 平均聚类损失
        """
    ...
    def kldloss(self, p, q):
        ...
        返回：
        - torch.mean(c1 - c2): 平均 Kullback-Leibler 散度损失
        """
        c1 = -torch.sum(p * torch.log(q), dim=-1)
        c2 = -torch.sum(p * torch.log(p), dim=-1)
        return torch.mean(c1 - c2)

    def pretrain_autoencoder(self, X1, X_raw1, sf1, X2, X_raw2, sf2, batch_size=256, lr=0.001, epochs=400, ae_save=True,
    ...
    def fit(self, X1, X_raw1, sf1, X2, X_raw2, sf2, lam1, lam2, y=None, lr=0.001, n_clusters=16, batch_size=256,
        ...
        for epoch in range(num_epochs):
            ...
            if epoch % self.t == 0:
                print("#Epoch %d: Total: %.6f Clustering Loss: %.6f ZINB Loss1: %.6f ZINB Loss2: %.6f KL Loss: %.6f" % (
                    epoch + 1, loss_val / num, cluster_loss_val / num, recon_loss1_val / num, recon_loss2_val / num,
                    kl_loss_val / num))
